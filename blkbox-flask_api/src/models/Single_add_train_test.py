...
SingleAdd = Blueprint(name='Single_add_train', import_name=__name__)

@SingleAdd.route('/train',methods=['GET'])
def train():
...
@SingleAdd.route('/test',methods=['GET'])
def test():
    data = request.get_json()
    given_img = data['img']
    
    ad_acc = re.search(r'.*?/(act.*?)-',given_img)
    ad_acc = ad_acc.group(1)
